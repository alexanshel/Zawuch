/******************************************************************************/
/***          Generated by IBExpert 2012.02.06 04.10.2015 19:30:53          ***/
/******************************************************************************/
...
CREATE OR ALTER PROCEDURE CURR_SUM (
    CURRID INTEGER)
RETURNS (
    GRYO INTEGER,
    GRC INTEGER,
    GRALL INTEGER,
    T0YO DOUBLE PRECISION,
    T1YO DOUBLE PRECISION,
    T2YO DOUBLE PRECISION,
    T3YO DOUBLE PRECISION,
    T4YO DOUBLE PRECISION,
    T5YO DOUBLE PRECISION,
    T6YO DOUBLE PRECISION,
    T7YO DOUBLE PRECISION,
    T8YO DOUBLE PRECISION,
    T0C DOUBLE PRECISION,
    T1C DOUBLE PRECISION,
    T2C DOUBLE PRECISION,
    T3C DOUBLE PRECISION,
    T4C DOUBLE PRECISION,
    T5C DOUBLE PRECISION,
    T6C DOUBLE PRECISION,
    T7C DOUBLE PRECISION,
    T8C DOUBLE PRECISION,
    T0A DOUBLE PRECISION,
    T1A DOUBLE PRECISION,
    T2A DOUBLE PRECISION,
    T3A DOUBLE PRECISION,
    T4A DOUBLE PRECISION,
    T5A DOUBLE PRECISION,
    T6A DOUBLE PRECISION,
    T7A DOUBLE PRECISION,
    T8A DOUBLE PRECISION,
    YTIMESUM DOUBLE PRECISION,
    OTIMESUM DOUBLE PRECISION,
    CTIMESUM DOUBLE PRECISION,
    ALLTIMESUM DOUBLE PRECISION)
AS
BEGIN
  SUSPEND;
END^
...
CREATE OR ALTER VIEW V_CURRICULUM(
    ...
    POS)
AS
SELECT c.id, c.name, c.period, c.pos
FROM curriculum  c
;
...
CREATE OR ALTER VIEW VIEW_CURRICULUM(
    ID,
    NAME,
    PERIOD_FOR_PRINT,
    PERIOD,
...
select d.id, d."Name", vc.period,
  ...
  vc.cat_5, vc.cat_6, vc.cat_7, vc.cat_8,
    sum((
...
from "Department" d
    inner join view_curriculum vc on
      d."CurriculumID" = vc.id
...
/* View: VIEW_CURRICULUM_RECORDS */
CREATE OR ALTER VIEW VIEW_CURRICULUM_RECORDS(
    ID,
    "CurriculumID",
    "Num",
    "SubjectID",
    ...
    SUMOT)
AS
select CR.id, CR.curr_id, CR.num, CR.subject_id, S."Name", CR.group_qty,
  ...
  sum(CRT.c_time), sum(CRT.c_time*C.category)
from curr_rec CR
...
group by cr.id, CR.curr_id, CR.num, CR.subject_id, S."Name", CR.group_qty
;
...
BEGIN
    SELECT
      SUM(coalesce(CRT.c_time, 0)) - SUM(coalesce(CRT.c_time, 0)*CC.category),
      SUM(coalesce(CRT.c_time, 0)*CC.category),
      SUM(case CRT.class_num when 0 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 1 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 2 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 3 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 4 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 5 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 6 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 7 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 8 then CRT.c_time else 0 end)
    FROM curr_cat CC join curr_rec CR on CC.curr_id = :currid and CR.curr_id = :currid
        join curr_rec_time CRT on CRT.curr_id = :currid and CR.id = CRT.curr_rec_id and CC.class_num = CRT.class_num
        join "Subject" S on S."ID" = CR.subject_id
    WHERE S."Code" <> 1
    INTO :YTimeSum, :OTimeSum, :T0YO, :T1YO, :T2YO, :T3YO, :T4YO, :T5YO, :T6YO, :T7YO, :T8YO;

    SELECT sum(CR.group_qty)
    FROM curr_rec CR join "Subject" S on CR.subject_id = S.id and CR.curr_id = :currid
    WHERE S."Code" <> 1
    INTO :GrYO;

    SELECT
      SUM(coalesce(CRT.c_time, 0)),
      SUM(case CRT.class_num when 0 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 1 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 2 then CRT.c_time else 0 end),
      SUM(case CRT.class_num when 3 then CRT.c_time else 0 end),
    ...
    FROM curr_cat CC join curr_rec CR on CC.curr_id = :currid and CR.curr_id = :currid
        ...
        join "Subject" S on S."ID" = CR.subject_id
    WHERE S."Code" = 1
    INTO :CTimeSum, :T0C, :T1C, :T2C, :T3C, :T4C, :T5C, :T6C, :T7C, :T8C;
    ...
    FROM curr_cat CC join curr_rec CR on CC.curr_id = :currid and CR.curr_id = :currid
        join curr_rec_time CRT on CRT.curr_id = :currid and CR.id = CRT.curr_rec_id and CC.class_num = CRT.class_num
        join "Subject" S on S."ID" = CR.subject_id
    INTO :AllTimeSum, :T0A, :T1A, :T2A, :T3A, :T4A, :T5A, :T6A, :T7A, :T8A;

    SELECT sum(CR.group_qty) FROM curr_rec CR WHERE CR.curr_id = :currid INTO :GrAll;
