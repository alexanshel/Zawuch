...
uses
  ...
  RxRichEd, dbcgrids, URTReport, DateUtils, ShellApi;
...
begin
  cbCurr.Clear;
  DM.ibdsCurriculum.First;
  while not DM.ibdsCurriculum.Eof do
  begin
    cbCurr.AddItem(DM.ibdsCurriculumNAME.Value, TObject(DM.ibdsCurriculumID.Value));
    DM.ibdsCurriculum.Next;
  end;
  cbCurr.ItemIndex := 0;
  cbCurr.OnChange(Self);
  dsCurr.DataSet := DM.ibdsCurrReport;
  dsCurrRec.DataSet := DM.ibdsCurrReportRec;
  dsCurrCat.DataSet := DM.ibdsCurrCat;
  SetSizes;
end;
