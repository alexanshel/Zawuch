object fmChangeTeacherSTP: TfmChangeTeacherSTP
  Left = 259
  Top = 188
  Width = 286
  Height = 276
  Caption = #1047#1072#1084#1077#1085#1072' '#1087#1088#1077#1087#1086#1076#1072#1074#1072#1090#1077#1083#1077#1081
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  PixelsPerInch = 96
  TextHeight = 20
  object RxLabel1: TRxLabel
    Left = 5
    Top = 8
    Width = 224
    Height = 20
    Caption = #1047#1072#1084#1077#1085#1103#1077#1084#1099#1081' '#1087#1088#1077#1087#1086#1076#1072#1074#1072#1090#1077#1083#1100
  end
  object RxLabel2: TRxLabel
    Left = 5
    Top = 88
    Width = 227
    Height = 20
    Caption = #1047#1072#1084#1077#1085#1103#1102#1097#1080#1081' '#1087#1088#1077#1087#1086#1076#1072#1074#1072#1090#1077#1083#1100
    FocusControl = edTeacher2
  end
  object BitBtn1: TBitBtn
    Left = 8
    Top = 190
    Width = 107
    Height = 40
    TabOrder = 0
    Kind = bkOK
  end
  object BitBtn2: TBitBtn
    Left = 151
    Top = 190
    Width = 116
    Height = 40
    Caption = #1054#1090#1084#1077#1085#1072
    TabOrder = 1
    Kind = bkCancel
  end
  object edTeacher1: TComboEdit
    Left = 5
    Top = 32
    Width = 265
    Height = 28
    DirectInput = False
    Glyph.Data = {
      
      E2E4E4E4E3E3E3E3E3E3}
    ButtonWidth = 23
    NumGlyphs = 1
    TabOrder = 2
    OnButtonClick = btnChoice1Click
  end
  object edTeacher2: TComboEdit
    Left = 5
    Top = 112
    Width = 265
    Height = 28
    DirectInput = False
    Glyph.Data = {
      
      E2E4E4E4E3E3E3E3E3E3}
    ButtonWidth = 23
    NumGlyphs = 1
    TabOrder = 3
    OnButtonClick = btnChoice2Click
  end
  object cbWGroup: TCheckBox
    Left = 8
    Top = 152
    Width = 201
    Height = 25
    Caption = #1089#1086' '#1074#1089#1077#1084#1080' '#1075#1088#1091#1087#1087#1072#1084#1080
    TabOrder = 4
  end
  object FormPlacement1: TFormPlacement
    Left = 24
    Top = 64
  end
end
